...
import { createCorrection } from "@/operations/service";
...
export async function POST(request: Request) {
  ...
  const parsed = correctionCreateRequestSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    ...
    return Response.json(correctionCreateResponseSchema.parse(result), {
      status: 201,
    });
  } catch (error) {
