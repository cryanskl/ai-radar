...
import { createSkillProfile } from "@/skills/service";
...
export async function POST(request: Request) {
  ...
  const parsed = skillProfileCreateRequestSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const result = await createSkillProfile(parsed.data);
    if (result.status === "invalid_reference") {
      return Response.json({ error: "invalid_reference" }, { status: 400 });
    }
    return Response.json(skillProfileCreateResponseSchema.parse(result), {
      status: 201,
    });
  } catch (error) {
