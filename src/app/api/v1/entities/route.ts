import { publicEntityListSchema } from "@/public-api/contracts";
import { localeSchema } from "@/events/contracts";
import { getPublicApiEnv } from "@/config/server-env";
import { listPublicEntitySummaries } from "@/public-api/service";
import {
...
export async function GET(request: Request) {
  ...
  const page = readPublicPageRequest(request, "entities", {
    locale: locale.data,
  });
  ...
  const result = await listPublicEntitySummaries({
    locale: locale.data,
